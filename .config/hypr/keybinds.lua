...
local frame_capture = home .. "/.config/quickshell/Scripts/Diagnostics/frame-capture.sh"
...
-- Brightness and volume repeat while held; the session lock blocks every global bind.
...
hl.bind("Print", hl.dsp.exec_cmd("grim - | wl-copy"),
    { description = "Utilities: Screenshot >> clipboard" })
...
hl.bind("SUPER + Backslash", hl.dsp.exec_cmd(stall_recorder .. " capture"),
    { description = "Utilities: Capture desktop stall" })
hl.bind("SUPER + F10", hl.dsp.exec_cmd(frame_capture .. " toggle"),
    { description = "Utilities: Screen frame capture (start/stop)" })
...
-- The wrapper suppresses live window capture before hyprlock takes the session lock.
local lock_cmd = home .. "/.config/quickshell/Scripts/Session/lock.sh"
...
hl.bind("SUPER + SHIFT + P", hl.dsp.exec_cmd("playerctl play-pause"),
    { description = "Media: Play/pause media" })
