...
GroupButton {
    ...
    property real previewWidth: Math.min(250, Math.max(140,
        ((parentGroup?.width ?? 550) - (parentGroup?.spacing ?? 0)) / 2 - Appearance.spacing.md))
    padding: Appearance.spacing.xs
    Layout.fillWidth: true
    colBackground: Appearance.colors.colLayer2
    toggled: Appearance.m3colors.darkmode === dark
    onClicked: {
        Quickshell.execDetached([Directories.wallpaperToolPath, "--mode", dark ? "dark" : "light", "--noswitch"])
    }
    contentItem: Item {
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                implicitWidth: lightDarkButtonRoot.previewWidth
                implicitHeight: skeletonColumnLayout.implicitHeight + Appearance.spacing.md * 2
