...
Item {
    ...
    readonly property int workspaceGroup: Math.floor(((monitor?.activeWorkspace?.id ?? 1) - 1) / ConfigOptions.bar.workspaces.shown)
    ...
    property int workspaceIndexInGroup: ((monitor?.activeWorkspace?.id ?? 1) - 1) % ConfigOptions.bar.workspaces.shown
