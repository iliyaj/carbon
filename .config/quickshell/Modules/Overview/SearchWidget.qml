...
Item { // Wrapper
    ...
    property var searchActions: [
        {
            action: "img",
            execute: () => {
                executor.executeDetached([Directories.wallpaperToolPath])
            }
        },
        {
            action: "dark",
            execute: () => {
                executor.executeDetached([Directories.wallpaperToolPath, "--mode", "dark", "--noswitch"])
            }
        },
        {
            action: "light",
            execute: () => {
                executor.executeDetached([Directories.wallpaperToolPath, "--mode", "light", "--noswitch"])
            }
        },
        {
            action: "accentcolor",
            execute: (args) => {
                const command = [Directories.wallpaperToolPath, "--noswitch", "--color"]
                if (args !== "")
                    command.push(args)
                executor.executeDetached(command)
            }
        },
        {
            action: "todo",
    ...
    Process {
        ...
        function executeDetached(command) {
