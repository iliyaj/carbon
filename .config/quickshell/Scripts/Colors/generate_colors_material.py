#!/usr/bin/env -S\_/bin/sh\_-c\_"source\_\$(eval\_echo\_\$CARBON_VIRTUAL_ENV)/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
import argparse
import math
import tomllib
...
parser.add_argument('--termscheme', type=str, default=None, help='TOML file containing the terminal scheme for generating term colors')
...
if args.termscheme is not None:
    with open(args.termscheme, 'rb') as f:
        term_source_colors = tomllib.load(f)['dark' if darkmode else 'light']
